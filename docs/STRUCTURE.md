...
```text
src/
  cli.ts                 command entrypoint (`pi-agent-hub`)
  index.ts               public exports
  app/                   dashboard launcher, controller, refresh loop, and shared session actions for the standalone TUI
  core/                  registry, paths, migration, tmux, Pi process args, status reducer, multi-repo workspace helpers
...
- Legacy local state from `<PI_CODING_AGENT_DIR>/pi-sessions` or `PI_SESSIONS_DIR` is migrated once by `src/core/migration.ts` before commands read registry state.
...
- `pi-agent-hub` opens a stable dashboard tmux session named `pi-agent-hub`; use `pi-agent-hub tui` to run the TUI directly without the dashboard wrapper.
...
- Pooled MCP uses a Unix socket at `<global-state>/pool/pool.sock`; run `pi-agent-hub mcp-pool` explicitly when pooled servers are enabled.
- Inside-tmux switch return state lives under `return-key/` while a temporary `Ctrl+Q` binding is active; `pi-agent-hub doctor` reports active/stale return state.
...
- Keep public names centralized in `src/core/names.ts`. Legacy `pi-sessions` strings should appear only in migration constants/tests or explicit migration warnings.
