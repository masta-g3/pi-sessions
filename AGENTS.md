...
- Groups are implicit session labels, not first-class records or selectable headers: `g` moves the selected session, `G` renames that session's current group label globally, and `e` renames the selected session.
